...
//	std::string get_url_address_from_console(); //lets user to enter url file in console
	...
	std::vector<std::string> get_lines_from_file(std::string filename); //gets list of html addresses from given file in every line should be one http_address
	int parse_html_and_save(const std::string& html_text, const std::string& node_of_html_tree, std::string& output,std::set<std::string> &unique_attribs); //parse html_text and save to file only the text from given node_of_html_tree
